    ...
    <main class="flex-1 flex flex-col bg-white overflow-y-auto">
        ...
        <div class="flex-1 bg-[#dcdcdc] p-8 m-8 mt-4 rounded-lg flex flex-col gap-8">
            ...
            <div class="bg-white p-6 rounded-md shadow-md">
                <h2 class="text-lg font-bold text-[#2a0a0a] mb-4">Tambah Mata Pelajaran</h2>
                <form action="{{ route('admin.mapel.store') }}" method="POST" class="flex gap-4 items-end">
                    @csrf
                    <div class="flex-1">
                        <label class="block text-gray-700 font-semibold mb-2">Nama Mata Pelajaran (Contoh: Matematika)</label>
                ...
                <table class="w-full text-left border-collapse text-white text-sm">
                    ...
                    <tbody>
                        ...
                        <tr class="hover:bg-[#454545] transition-colors">
                            ...
                            <td class="p-4 border border-gray-600 font-bold">{{ $mapel->name }}</td>
                            <td class="p-4 border border-gray-600 text-center">
                                <form action="{{ route('admin.mapel.destroy', $mapel->id) }}" method="POST" onsubmit="return confirm('Hapus mata pelajaran ini?');">
                                    @csrf
                                    @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="3" class="p-6 border border-gray-600 text-center text-gray-400">Belum ada data mata pelajaran.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
