...
<body class="bg-gray-100 font-sans antialiased flex h-screen overflow-hidden">
    ...
    <aside class="w-72 bg-[#2a0a0a] text-white flex flex-col h-full shrink-0">
        <div class="flex flex-col items-center justify-center p-8 mt-12">
            ...
            <p class="text-sm text-gray-300 mt-1">{{ auth('teacher')->user()->subject ?? 'Mata Pelajaran' }}</p>
        </div>
    ...
    <main class="flex-1 flex flex-col bg-white overflow-y-auto">
        ...
        <div class="flex-1 bg-[#dcdcdc] p-8 m-8 mt-2 rounded-lg flex flex-col">
            ...
            <form method="GET" action="{{ route('guru.portfolio') }}" class="flex flex-col gap-3 w-64 mb-6">
                <!-- Static category for now -->
                <select class="border rounded px-3 py-2 bg-white text-black font-semibold focus:outline-none">
                    <option>Pengajaran</option>
                </select>

                <!-- Dynamic Kelas Selection -->
                <select name="kelas" onchange="this.form.submit()" class="border rounded px-3 py-2 bg-white text-black font-semibold focus:outline-none">
                    <option value="">-- Pilih Kelas --</option>
                    @foreach($kelasList as $kelas)
                        <option value="{{ $kelas->name }}" {{ $selectedKelas == $kelas->name ? 'selected' : '' }}>
                            {{ $kelas->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="bg-[#333] rounded overflow-hidden shadow-lg border border-[#444] flex-1 max-h-[60vh] overflow-y-auto">
                <form method="POST" action="{{ route('guru.portfolio.store') }}">
                    @csrf
                    <!-- Pass the semester safely to the POST request -->
                    ...
                    <table class="w-full text-left border-collapse text-white text-sm">
                        ...
                        <tbody>
                            @if($selectedKelas && $selectedSemester && $students->count() > 0)
                                @foreach($students as $index => $student)
                                    ...
                                    <tr class="hover:bg-[#454545] transition-colors border-b border-[#555]">
                                        ...
                                        <td class="p-3 border-r border-[#555]">{{ $student->name }}</td>
                            ...
                            @else
                                <tr>
                                    <td colspan="6" class="p-6 text-center text-gray-400">
                                        {{ (!$selectedKelas || !$selectedSemester) ? 'Silakan pilih Kelas dan Semester terlebih dahulu.' : 'Tidak ada siswa di kelas ini.' }}
                                    </td>
                                </tr>
                            @endif
                        </tbody>
